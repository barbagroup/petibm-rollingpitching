...
ring1 = get_midpoint(Point(231, 232), Point(340, 201))  # first vortex ring
ring2 = get_midpoint(Point(467, 155), Point(547, 173))  # second vortex ring
alpha = Line(ring1, ring2)  # line passing through the vortex ring centers
print(f'alpha: {alpha.get_inclination():.2f}')
line = alpha.extend(left=100, right=100)
...
annotate_angle(ax, r'$\alpha$', 400, line, hline)

# Compute and plot inclination line for beta.
ring = (Point(358, 429), Point(446, 408))  # vortex ring extremities
beta = Line(*ring)
print(f'beta: {beta.get_inclination():.2f}')
line = beta.extend(left=200, right=20)
ax.plot(*line.limits(), color='black', linestyle='--', linewidth=3.0)
hline = Line(line.p1, Point(line.p2.x, line.p1.y))
ax.plot(*hline.limits(), color='black', linestyle='-.', linewidth=3.0)
annotate_angle(ax, r'$\beta$', 450, line, hline)
